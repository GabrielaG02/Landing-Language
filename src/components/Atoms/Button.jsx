...
const Button = ({ text, onClick, tipo = 'primario', iconName, disabled = false }) => {
  // Clases base
  const baseClasses =
    "font-light  shadow-md focus:outline-none focus:ring-1 transition-colors duration-300 focus:border-[#FB91DB] focus:border-2 focus:bg-(--color-primary)";

  let colorClasses = "";
  if (tipo === 'primario') {
    colorClasses = "bg-(--color-primary) hover:bg-(--color-light-700) text-white rounded-full px-8 py-2";
